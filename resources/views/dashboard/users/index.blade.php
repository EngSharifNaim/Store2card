...
{{--                                            </a>--}}
                                        ...
                                        <td>
                                            <form id="promo_form_{{$user->id}}">
                                                @csrf
                                                <div class="input-group input-group-sm" style="width: 150px;">
                                                    <input type="text" hidden name="user_id" value="{{$user->id}}">
                                                    <select name="category" class="form-control float-right" placeholder="Search">
                                                    @foreach(\App\CoponCategory::all() as $category)
                                                        <option value="{{$category->id}}">{{$category->name}}</option>
                                                        @endforeach
                                                    </select>

                                                    <div class="input-group-append">
                                                        <button type="button" onclick="send_promo({{$user->id}})" class="btn btn-default">ارسال كوبون</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </td>
...
function  send_promo(id)
    {
        $.ajax({
            url:'{{url("send_promo")}}',
            method:'post',
            data:  new FormData(document.getElementById('promo_form_' + id)),
            // dataType:'json',
            contentType: false,
            cache: false,
            processData:false,
